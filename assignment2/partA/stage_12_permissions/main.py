...
Every message that enters or leaves `messages` also gets logged to a
JSONL file via sessions.log_message, in the same spot it's appended to
`messages`. `/rewind` and any other slash command are intercepted at the
top of the outer loop and handled locally -- they never get appended to
`messages` and never reach client.chat.completions.create.
"""
import json
import os
import sys
