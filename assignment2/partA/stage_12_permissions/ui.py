"""Stage 10: all screen drawing lives here.
...
def banner(model_name):
    ...
    console.print("type your message, /rewind to undo the last turn, Ctrl+C to quit\n")
...
def print_usage(prompt_tokens, completion_tokens, total_tokens, cached_tokens=None):
    ...
    console.print(
        ...
        f"total={total_tokens} cached_tokens={cached}[/dim]"
    )


def print_system(text):
    """For local, non-model status lines: session ids, /rewind confirmation,
    unknown-command notices. Never sent to the model."""
