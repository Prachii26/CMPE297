...
Two additions to stage 13's run_loop, both from compaction.py:

1. Every tool result passes through cap_fresh_result before it's stored,
   unconditionally -- a single oversized result gets capped no matter how
   full the window is.
2. compact() runs once at the top of every loop iteration, before the
   request goes out, and only does anything once the transcript has
   crossed 85% of the (approximate) context window.
"""
import json
import os
import sys
...
def run_loop(client, messages, session_id):
    for _ in range(MAX_TURNS):
        # Checked before every single request. A no-op unless the
        # transcript has crossed HIGH_WATER -- see compaction.py.
